...
<body>
  <div id="error-overlay"><h1>⚠️ Prototype Error</h1><div id="err-msg"></div></div>

  <div id="app">
    <!-- top row -->
    <div id="top-left">Initializing…</div>
    <div id="top-right"></div>

    <!-- middle row -->
    <div id="battlefield"></div>
    <div id="log-wrap">
      <div id="combat-log"><h3>📜 Combat Log</h3></div>
    </div>
...
__setWithVariants('mag',  'Premonition',   {proc:false});  // status only; type won’t be used
...
case 'Warrior': return [
  ...
  {label:'Warhorn', desc:'Heal and briefly bolster yourself with strength and quickness.',
   ...
   use:(a)=>{
     // ATK↑ — 1 stack for 3 rounds
     const atkDur = 3;
     pushStatus(a, { key:'atk_up', dur: atkDur });

     // Premonition — acts at fast speed next round
  ...
  {label:'Shieldwall', desc:'Allies gain mitigation; intercept first hit.', speed:'fast', type:'phys', target:'all-allies',
   use:(a,arr)=>{
    ...
    case 'Barbarian': return [
      ...
      {label:'Rend', desc:'Expose your target’s weakness.', speed:'slow', type:'phys', target:'enemy',
       base:{min:20,max:80}, statCoef:0.35,
       use:function(a,t){ const r=hit(a,t,{ability:this});
         if(t.alive()){ const dur=buffDurBySDM(a); pushStatus(t,{key:'def_down', dur}); appendLog(`${t.name}’s defense is lowered (${dur}).`);} return r; } },
      ...
      {label:'War Cry', desc:'Damage all enemies and lower their defense.', speed:'normal', type:'mag', target:'all-enemies',
       ...
       use:function(a,arr){ for(const t of arr){ if(!t.alive()) continue; hit(a,t,{ability:this}); }
         ...
         appendLog(`All enemies’ defense is reduced (${dur}).`); } },
      ...
      {label:'Rampage', desc:'Swing that grants rampage on killshot.', speed:'normal', type:'phys', target:'enemy',
       ...
       use:function(a,t){
         ...
         if (hpBefore>0 && t.hp<=0){
           ...
           appendLogHTML(`<span style="color:#ff927a">${a.name} goes on a Rampage! ATK↑ (${dur}).</span>`);
         }
         return r;
       }}
    ];
...
case 'Cleric': return [
  {
    ...
    use: (a, t) => {
      ...
      appendLog(`${a.name} blesses ${t.name} with a Defensive Aura: DEF↑ (${dur}), healed for ${healed}.`);
    }
  },
...

  {
    ...
    use: (a, arr) => {
      for (const ally of arr){
        ...
        appendLog(`${a.name} channels Enid’s Blessing: ${ally.name} healed for ${ally.hp - before} and cured of poison.`);
      }
    }
  },
...

  {
    ...
    use: (a, t) => {
      ...
      if (t.alive()){
        ...
        t.hp = clamp(t.hp + H, 0, t.maxHP);
        t._refresh && t._refresh(); checkHPWarnings(t);
        appendLog(`${t.name} is already alive — healed for ${t.hp - before}.`);
      } else {
        t.hp = Math.max(1, Math.floor(t.maxHP * 0.35));
        removeStatusKey(t, 'poison');
        t._refresh && t._refresh(); checkHPWarnings(t);
        appendLogHTML(`<span style="color:#a7c4ff">✝ ${a.name} resurrects ${t.name} with ${t.hp} HP!</span>`);
        a._rezUsed = true;
      }
    }
  },
 ...
 use:function(a,t){
  ...
  const healed = t.hp - before;
  t._refresh && t._refresh(); checkHPWarnings(t);
  appendLog(`${a.name} focuses ${t.name}: ATK↑ (${dur}), healed for ${healed}.`);
}},
    ...
    case 'Necromancer': return [
      ...
      {label:'Life Drain', desc:'Steal life from a foe.', speed:'normal', type:'mag', target:'enemy',
       ...
       use:function(a,t){
         ...
         if (res.crit){
           appendLogHTML(`${a.name}’s spell lands a <span style="color:gold">critical</span> drain! ${t.name} loses ${dealt}. ${a.name} is healed for ${dealt}.`);
         } else {
      ...
      {label:'Death Touch', desc:'Crush half the foe’s current life.', speed:'slow', type:'mag', target:'enemy',
       use:(a,t)=>{
    ...
    case 'Mystic': return [
      ...
      {label:'Premonition', desc:'All ally actions become fast next round.', speed:'fast', type:'mag', target:'all-allies',
       use:(a,arr)=>{ for (const ally of arr){ if(ally.alive()) pushStatus(ally,{key:'premonition', dur:2}); }
         appendLogHTML(`${a.name} invokes <b>Premonition</b> — your party acts faster next round.`); } }
    ];
...
function computeAbilityHeal(caster, target, ability){
  ...
  // buffed stats affect heals too (SDM↑/ATK↑)
...
function maybeProcOnBasic(attacker, target){
  ...
  switch(attacker.job){
    case 'Cleric':{
      ...
      appendLogHTML(`<span style="color:#a7c4ff">✨ A prayer mends ${attacker.name} for ${total}.</span>`); break;
    }
    case 'Mage':{
      ...
      const r=computeAbilityDamage(attacker,target,bolt); if(r.hit){ target.hp=clamp(target.hp-r.dmg,0,target.maxHP);
        ...
        appendLogHTML(`<span style="color:#a7c4ff">⚡ A lightning bolt zaps ${target.name} for ${r.dmg}!</span>`); }
      break;
    }
    case 'Monk':{ const dur=buffDurBySDM(attacker); pushStatus(attacker,{key:'atk_up',dur});
      appendLogHTML(`<span style="color:#a7c4ff">🌀 ${attacker.name} finds an opening: ATK↑ (${dur}).</span>`); break; }
    case 'Mystic':{ if(!target||!target.alive()) return; const dur=buffDurBySDM(attacker);
      pushStatus(target,{key:'def_down',dur}); appendLogHTML(`<span style="color:#a7c4ff">🪄 ${target.name}'s guard falters: DEF↓ (${dur}).</span>`); break; }
    case 'Necromancer':{
      ...
      if(r.hit){ target.hp=clamp(target.hp-r.dmg,0,target.maxHP); attacker.hp=clamp(attacker.hp+r.dmg,0,attacker.maxHP);
        ...
        appendLogHTML(`<span style="color:#a7c4ff">🩸 A dark sip steals ${r.dmg} from ${target.name}.</span>`); }
      break;
    }
    case 'Paladin':{ const dur=buffDurBySDM(attacker); pushStatus(attacker,{key:'def_up',dur});
      appendLogHTML(`<span style="color:#a7c4ff">🛡️ ${attacker.name}'s resolve hardens: DEF↑ (${dur}).</span>`); break; }
  }
}
...
function updateStatusView(u){
  ...
  ui.statsTxt.setText(`ATK ${effATK.toFixed(1)}  •  SDM ${effSDM.toFixed(1)}  •  DEF ${effDEF.toFixed(1)}`);
  ...
  const labelFor=(key)=>(
    key==='def_up'?'DEF↑': key==='def_down'?'DEF↓': key==='atk_up'?'ATK↑': key==='sdm_up'?'SDM↑':
  ...
  for (const key of present){
    ...
    if (STACKABLE.has(key)){ const n=countStacks(u,key); if(n>1){ const sTxt=scene.add.text(0,0,`×${n}`,{fontSize:`${Math.max(10,Math.round(ICON_SIZE*0.6))}px`,color:'#ffd37a'}); sTxt.setOrigin(1,1); sTxt.setPosition(x+ICON_SIZE-2,y+ICON_SIZE-2); ui.panel.add(sTxt); ui.statusNodes.push(sTxt); } }
...
/* === CANONICAL ORDER HELPERS (host→guest, then by name) === */
...
function hit(a, t, opt={}, verb){
  ...
  if (res.dodged){ appendLogHTML(`${target.name} <span style="color:skyblue">dodges</span> ${a.name}’s ${abilityName}!`); return {dealt:0,dodged:true,crit:false}; }
  ...
  if (abilityName.toLowerCase()==='attack'){
    if (res.crit) appendLogHTML(`${a.name}’s attack lands a <span style="color:gold">critical strike</span>! ${target.name} takes ${dmgToApply} damage.`);
    else          appendLog(`${a.name} attacks ${target.name} for ${dmgToApply} damage.`);
  } else {
    if (res.crit) appendLogHTML(`${a.name}’s ${abilityName} lands a <span style="color:gold">critical strike</span>! ${target.name} takes ${dmgToApply} damage.`);
    else          appendLog(`${a.name} uses ${abilityName}. ${target.name} takes ${dmgToApply} damage.`);
  }

  if (dmgToApply>0 && target.hp<=0){
  appendLogHTML(`<span style="color:#ff6b6b">☠ ${target.name} is down!</span>`);
...
/* ===== PART 3/4 — Parties, UI, Scene, Round ===== */
...
function updateTopBarForCurrentSelection(){
    // If we’re in the lobby, force the title (no round text)
  if (PVP && PVP.phase === 'idle'){
    setTopBarLobby();
    return;
  }
const actor = currentActor();
  if (!actor){ topLeftEl.textContent = `Round ${roundNumber} — Waiting…`; return; }
  ...
  topLeftEl.textContent = `Round ${roundNumber} — Select ${tgtWord} for ${actor.name}'s ${abLabel}`;
  return;
}


  const ta = actor.status?.find(s=>s.key==='taunt' && s.dur>0 && s.data?.taunter?.alive());
  if (ta){ topLeftEl.textContent = `Round ${roundNumber} — ${actor.name} is taunted: must Attack ${ta.data.taunter.name}`; return; }

  topLeftEl.textContent = `Round ${roundNumber} — Choose action for ${actor.name}`;
}
...
function canonicalKey(u){
  // Use network-side + name so it’s the same on both clients
...
async function resolveSimultaneous(){
  ...
  // canonical action order: speed → QKN (desc) → stable tiebreaker
  q.sort((A, B) => {
...
if (['enemy','ally'].includes(ability.target)){
  ...
  if (eAlive.length===0){ appendLog('🏆 Victory!'); return 'win'; }
  if (pAlive.length===0){ appendLog('💀 Defeat.');  return 'lose'; }
  return 'continue';
}
...
if (PVP && PVP.phase === 'battle' && PVP.waitingForOpp) {
  ...
  actionPromptEl.textContent = `Round ${roundNumber} — Waiting for opponent…`;
...
_pendingAbility = null;
  ...
  if (selectingIndex >= alive.length){
    ...
    if (topLeftEl) topLeftEl.textContent=`Round ${roundNumber} — Waiting for opponent…`;
    return;
  }
...
ensureUnitSides();
    ...
    appendLog(`[DBG] Scene.create — player=${player.length}, enemy=${enemy.length}, picksMe=${PVP?.picksMe?.length||0}, picksOpp=${PVP?.picksOpp?.length||0}`);
    ...
    if ((player.length === 0 || enemy.length === 0) &&
      ...
      appendLog('[DBG] Forcing hydrate from picks after restart…');
      player = PVP.picksMe.map(job => makeUnit(job, 'player'));
      enemy  = PVP.picksOpp.map(job => makeUnit(job, 'enemy'));
      this.rebuildPanels();
    }
    ...
    // round UI (safe even if empty—buttons will be hidden by waiting state)
    roundNumber = 1;
    resetRoundState();
    updateTopBarForCurrentSelection();
...
function refreshLobbyList(rooms){
  ...
  for (const r of rooms){
    ...
    const info = el('div', {}, `${r.id} — ${r.name || 'Room'} (${r.players || 1}/2)`);
...
function setLobbyRoomStatus(id){
  ...
  if (id){
    ...
    n.textContent = `Room ${id} created. Waiting for opponent…  (Share this ID to invite friends)`;
  } else {
...
function connectWS(){
  ...
  _wsOpenPromise = new Promise((resolve)=>{
    ...
    const ws = new WebSocket(RELAY_URL);   // <— changed line
    PVP.ws = ws;
    // …keep the rest of your connectWS() exactly as you have it…
  ...
  try {
    const msg = JSON.parse(ev.data);  // turn the text into an object
    handleNet(msg);                   // send it to your handler
  } catch (err) {
...
(function(){
  ...
  function sendMyRoundChoices(){
    ...
    if (topLeftEl) topLeftEl.textContent = `Round ${r} — Waiting for opponent…`;
    if (typeof clearActionBar === 'function') clearActionBar();
  ...
  async function tryResolveRoundWithBothSides(r = roundNumber){
    ...
    // seed RNG from both sides’ wires so both clients match
    ...
    seedRNG(roundSeed);
    appendLogHTML(`<span style="color:#ffd37a">— Round ${r} —</span> <span style="opacity:.7">(seed ${roundSeed})</span>`);
...
function handleNet(msg){
  switch(msg.kind){
    ...
    case 'queued': {
      appendLog('[PvP] Searching for opponent…');
    ...
    case 'room_joined': {
      appendLog('[PvP] Joined room. Waiting for opponent…');
      break;
    }
    case 'error': {
...
function getOrCreateDraftOverlay(){
  ...
  const poolHeader=document.createElement('div'); poolHeader.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
      ...
      <input id="draft-filter" placeholder="Filter classes…" style="background:#22314b;color:#fff;border:1px solid #4a5a7a;border-radius:4px;padding:6px 8px;min-width:180px;">
    </div>`;
  ...
  footer.innerHTML=`<div>Alternating draft (A–B–A–B–A–B). Battle starts when each player has 3 picks.</div><button id="draft-cancel" class="action-btn" style="padding:6px 10px;">Cancel</button>`;
...
function mountDraftUI(){
  ...
  const box=document.getElementById('action-bar-right');
  if (box){ box.textContent=''; const hint=document.createElement('div'); hint.style.cssText='font-size:12px;opacity:.85;text-align:center;padding:8px;'; hint.textContent='Draft in progress…'; box.appendChild(hint); }
  updateDraftUI();
}
...
function updateDraftUI(){
  ...
  if (myP) myP.textContent=(PVP.picksMe.join(', ')||'—'); if (opP) opP.textContent=(PVP.picksOpp.join(', ')||'—');
  ...
  for (const job of remaining){
    ...
    const desc=document.createElement('div'); desc.className='desc'; desc.textContent=JOBS[job]?`STR ${JOBS[job].STR} • INT ${JOBS[job].INT} • QKN ${JOBS[job].QKN} • TGH ${JOBS[job].TGH}`:'';
